...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe, gcaptcha_sitekey }){
  let div = dom('.splash', dom('Frontera'), dom('We are an open community for emerging markets finance and investment'), !iframe && dom('.logos', logo && dom('.logo.org'), dom('.logo.slack')
    ),
    dom('p',
      'Join ', dom('b', name),
